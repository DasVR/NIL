...
| Task-complete — check draws in (doesn't pop) | 4 | partial | `lib/styles/motion.css` (`nil-draw` / TRACE primitive #09) | The stroke-draw mechanism exists (used as the agent spine), but there is no dedicated task-complete checkmark demo. |
| Invalid-input shake | 4 | no | — | No shake keyframe/primitive in the app. Not required by prompts 2–6. |
