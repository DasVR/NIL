...
Built with **Svelte 5**, **TypeScript**, and **SvelteKit static adapter**. The UI is designed to feel like a macOS app: greyscale chrome, severity color, spring physics, and a keyboard-first command palette.

![Empty state](./frontend/screenshots/empty-state.png)

## Screenshots

| Empty state | Composer (Cmd+J) | Command palette (Cmd+K) |
|---|---|---|
...
- `frontend/` — SvelteKit app
  - `src/lib/components/shell/` — workstation shell (sidebar, stream, inspector, composer, status bar, command palette, settings)
  - `src/lib/components/ui/NilMonogram.svelte` — Zone A identity mark (cold open, lock, handoff, report cover)
  - `src/lib/components/ui/` — tool cards, findings, window chrome
