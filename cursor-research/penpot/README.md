...
- Secrets are **not** committed. `./start.sh` writes a gitignored `.env` with a generated `PENPOT_SECRET_KEY` and database password. To start the bridge stack yourself: `docker compose --env-file .env -f docker-compose.yaml up -d`.
